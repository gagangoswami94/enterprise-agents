#!/usr/bin/env bash
#
# install.sh -- Install Enterprise Agents agents into your local agentic tool(s).
#
...
#   gemini-cli   -- Install extension to ~/.gemini/extensions/enterprise-agents/
#   opencode     -- Copy agents to .opencode/agent/ in current directory
#   cursor       -- Copy rules to .cursor/rules/ in current directory
#   aider        -- Copy CONVENTIONS.md to current directory
#   windsurf     -- Copy .windsurfrules to current directory
#   openclaw     -- Copy workspaces to ~/.openclaw/enterprise-agents/
...
interactive_select() {
  ...
  while true; do
    ...
    printf "\n"
    box_top
    box_row "${C_BOLD}  Enterprise Agents -- Tool Installer${C_RESET}"
    box_bot
    printf "\n"
...
install_gemini_cli() {
  ...
  local dest="${HOME}/.gemini/extensions/enterprise-agents"
  local count=0
...
install_openclaw() {
  ...
  local dest="${HOME}/.openclaw/enterprise-agents"
  local count=0
...
main() {
  ...
  if $use_interactive; then
    ...
    header "Enterprise Agents -- Scanning for installed tools..."
    printf "\n"
    local t
    for t in "${ALL_TOOLS[@]}"; do
      if is_detected "$t" 2>/dev/null; then
  ...
  printf "\n"
  header "Enterprise Agents -- Installing agents"
  printf "  Repo:       %s\n" "$REPO_ROOT"
